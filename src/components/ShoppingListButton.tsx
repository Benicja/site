  ...
  return (
    <>
      <div className={`group ${className} relative`}>
        <button
          className="w-14 h-14 md:w-12 md:h-12 rounded-full shadow-[0_0_20px_rgba(0,0,0,0.5)] transition-all hover:scale-110 active:scale-95 focus:outline-none bg-black text-white flex items-center justify-center"
          onClick={handleOpen}
          aria-label="Open shopping list"
        >
          <svg className="w-7 h-7 md:w-6 md:h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
        ...
        {badgeCount > 0 && (
          <div className="absolute -top-1 -right-1 w-6 h-6 md:w-5 md:h-5 bg-white text-black border-2 border-black rounded-full flex items-center justify-center text-xs md:text-[10px] font-bold pointer-events-none">
            {badgeCount}
          </div>
        )}
      </div>
